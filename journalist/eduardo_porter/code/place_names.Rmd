...
```{r, message=FALSE}
raw_borja_2000 <- read_dta(paste0(dropbox_path, "BORJAS2000FINAL.dta"))

borja_2000 <- raw_borja_2000 %>%
  select(lincShared, lr, skill, statefip, pumares2mig)

ipums <- read_excel("../src/ipums_usa_puma_migpuma_2000.xlsx")

puma_names <- 
  read_table("../src/2000PUMAsASCII.txt", 
...
borja_2000 %>%
  ...
  left_join(statefip, by = "statefip") %>%
  filter(skill == 0) %>%
  arrange(desc(lr)) %>%
  transmute(name, stname,
            household_inc_everyone = round(exp(lincShared)), 
            household_inc_minus_housing_lowskill = round(exp(lr))) %>% 
  write_excel_csv("../out/inc_by_skill_by_place.csv")
```
...
msa_data %>%
  mutate(lr_hat = predict(model, lincShared),
         resid = lr - lr_hat) %>%
  filter(pop > 1000000) %>% arrange(desc(resid)) %>% select(-skill, -msa, -top_6) %>% 
